...
def verify_employment_id(db: Session, employment_id: str) -> Employee | None:
    ...
    no match. Raises on a genuine DB error - callers (app.relay.group_pipeline,
    app.relay.personal_pipeline) must treat any exception here as "cannot
    verify" and fail closed, not as a match."""
    return db.execute(
