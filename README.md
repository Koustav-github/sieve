...
connection). It needs real Caspian credentials, plus an Anthropic API key for
the relay-detection LLM call (a single structured-output call that detects
whether an inbound message is asking to be relayed to another team and, if
so, extracts who it should go to and what to tell them - see
`apps/server/app/relay/pipeline.py` and `apps/server/app/relay/llm.py`):
copy `.env.example` to a root `.env` (gitignored) and fill in
`CASPIAN_API_KEY`, `CASPIAN_BASE_URL`, `TELEGRAM_BOT_TOKEN`,
